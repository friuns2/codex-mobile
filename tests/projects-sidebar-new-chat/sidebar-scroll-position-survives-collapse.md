...
3. Collapse the sidebar with the sidebar toggle, `Command+B` on macOS, or `Control+B` on non-macOS platforms.
4. Reopen the sidebar with the header/sidebar toggle, `Command+B` on macOS, or `Control+B` on non-macOS platforms.
5. On macOS, press `Control+B` and confirm it does not collapse or reopen the sidebar.
6. Confirm the same lower project or thread row is still near the top and the sidebar did not jump back to the top.
7. Repeat collapse/reopen twice quickly and confirm the restored position is not clobbered to the top while collapsed.
8. Repeat the collapse and reopen flow in dark theme.
...
- `Command+B` toggles the sidebar on macOS, `Control+B` toggles it on non-macOS platforms, and `Control+B` does not toggle it on macOS.
